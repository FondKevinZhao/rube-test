...
new 的本质是生成一个新对象，将对象的`__proto__`指向函数的 prototype，再执行call 方法。
...
- 在当前页面发送请求的时候，在 axios 发送请求的时候，在配置项 cancelToken 中：new axios.cancelToken((c)=>{})，回调函数中会自动传入一个当前请求的标识C，以数组的形式将当前请求保存到 vuex 中。
