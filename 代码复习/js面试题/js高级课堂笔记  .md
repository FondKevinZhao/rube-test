...
83. **promise 是什么？作用？所有的方法？**

    **定义：**promise 是一个异步编程的解决方案，将异步操作以同步的形式表达出来，避免回调地狱方式的出现。

     **promise 对象：**

    1. Promise 是一个构造函数，需要实例化调用，得到一个实例化的 promise 对象。

    2. Promise 对象上有静态方法：all、race、reject、resolve 等。

    3. Promise 原型对象上有：catch、finally、then。

     **promise 的使用：**

    1. promise 接受一个函数作为参数，把异步代码写入这个函数中即可。

    2. promise 是处理异步代码的，但是本身是同步执行的。

     **promise 的函数的两个参数：**

    1. resolve：成功回调函数，当异步处理成功了，则直接调用成功回调函数。

    2. reject：失败回调函数，当异步处理失败了，则直接调用失败回调函数。

    3. 无论调用 resolve 还是 reject，都会把结果返回给 promise 对象。

    4. resolve 和 reject 方法调用的时候，可以传递参数，把成功的结果或失败的报错信息返回给 promise 对象接收。

    **promise 的三个状态：**pending(初始化状态)、resolved/fulfilled(成功状态)、rejected(失败状态)。

     **promise 实例化对象：**

    1. pending：初始化状态，异步代码正在执行中，或者开发人员没有调用 resolve 或 reject。

    2. resolved/fulfilled：成功状态，只要在 promise 中调用了 resolve 即可返回成功状态。

    3. rejected：失败状态，当 promise 调用了 reject() 方法的时候，返回失败状态。

    4. [[PromiseResult]]：promise 内部成功或失败的时候，调用 resolve 或 reject 方法，给的参数。
    ...
    **谈一谈 promise 的原型方法**
    ...
    1. 可以捕获 promise 对象的成功状态，然后再执行下一步回调函数。

    2. then方法接受一个函数作为参数 then 的返回值。

    3. 如果穿透了(then中不书写回调函数)，则一定返回成功。

    4. 如果 then 内部出现错误信息，则返回失败 promise，值为错误信息。

    5. 其他情况只要看 then 回调函数 return 的 promise 对象即可。

    6. then 回调函数的参数就是调用 then 的成功 promise 对象的 promiseResult 的值。 

    

     promise.prototype.catch():

    1. 可以捕获 promise 对象的失败状态，然后对错误处理.

    2. catch 的返回值：
       - 如果穿透了(catch 中不书写回调函数)，则一定返回失败。
       - 如果 catch 内部出现错误信息，则返回失败的 promise，值为错误信息。
       - 其他情况只要看 catch 回调函数 return 的 promise 对象即可。
       - catch 回调函数的参数就是调用 catch 的失败 promise 对象的 promiseResult 的值。

    

    promise.finally():

    - 可以捕获成功或失败的 promise 状态，一般作为整个 promise 异步操作的最后一步。

    - promise.then 和 catch 如果不书写任何参数，则会直接把调用 then 或 catch 的 promise 对象穿透过来并返回。

      

    **async...await**

    这是 es8 出来的语法，写法比 .then .catch 写法更加优雅。

    await 之后的代码，会被自动放在 .then 内部。

    await 之后可以书写任何内容，但是如果是非 promise 对象，就失去 async 的意义了。

    写了 await 就一定要写 async 否则控制台会报错。

    如果想要实现类似 catch 效果，可以在 async 函数内部使用 try{}catch (error){...}，它会被自动编译成 .catch。

    **try {...} catch (error){...}** ： 一般用作不在乎这个是否错误，正常往下运行：

    try {...}：尝试着解析，如果没问题就正常运行。

    catch (error){...}：如果出错，就提示出来。
...
87. 谈谈作用域
    ...
    **作用域定义：**变量起作用的范围。运行代码时，某些特定部分中的变量，对函数和对象的可访问性。换句话说，作用域决定了代码区块中变量和其他资源的可见性。

    **作用：**隔离变量，不同作用域下同名变量不会有冲突。es6 之前没有块级作用域，只有全局作用域和函数作用域。es6 中可以通过 let 和 const 来说实现块级作用域。

    **四个作用域：**全局作用域、函数作用域(局部作用域)、eval() 作用域、块级作用域。
    ...
    **注意：**作用域是分层的，内层作用域可以访问外层作用域的变量，反之则不行。

    块级作用域：块级作用域可通过新增命令 let 和 const 声明，所声明的变量在指定块的作用域外无法被访问。
    ...
    let：变量，一开始可不赋值，可重复声明。

    const：常量，一开始就要赋值，不可重复声明，一般要求名字大写(可不遵守)。

    **作用域与执行上下文的区别：**

    1. 作用域在定义时就确定了，并且不会改变。执行上下文在运行时确定，随时可能改变。
    2. 一个作用域下可能包含若干个上下文环境。有可能从来没有过上下文环境（函数从来就没有被调用过）；有可能有过，现在函数被调用完毕后，上下文环境被销毁了。
    3. 有可能同时存在一个或多个（闭包）。**同一个作用域下，不同的调用会产生不同的执行上下文环境，继而产生不同的变量的值**。

    [博客地址](https://www.cnblogs.com/leftJS/p/11067908.html)

88. 谈谈作用域链

    **定义：**寻找变量的过程。

    **过程：**一开始会在自身所在的作用域中查找，如果没有就会一层一层向上查找，如果找到全局作用域还是没有找到，就会宣布 xxx is not defined。这种一层一层查找的过程就是作用域链。
...
89. **谈谈原型（尽可能多说）**
    ...
    **总**
    ...
    每一个函数都有一个显示原型属性 prototype。

    每一个对象都有一个隐式原型属性 `__proto__`。

    **分**

    1. 除了箭头函数以外的其他函数都有显示原型属性，指向它的原型对象，原型对象上有 constructor 指向函数本身。

    2. 构造函数通过 new 出来的实例化对象，它的`__porto__`指向其构造函数的原型对象。

    3. Function 可以看作是 new 自己产生的，所以 Function 的隐式原型指向 Function 的显示原型，他们的值指向同一个对象。

    4. `Object.prototype.__proto__ === null` 原型链的尽头。

    数组都是 new Array 产生的，Array 原型对象上 filter、map、reduce 等方法，所以所有数组的实例都能通过隐式原型继承得到这些方法。

    函数都是 new Function 产生的，Function 原型对象上 call、apply、bind 等方法，所以所有函数的实例都能通过隐式原型继承得到这些方法。
    ...
    **原型链**
    ...
    2. 沿着隐式原型属性一层层查找，找到就返回值。

    3. 最后来到`Object.prototype.__proto__`, 它的值是 null，就不找到了，返回值是 undefined。
