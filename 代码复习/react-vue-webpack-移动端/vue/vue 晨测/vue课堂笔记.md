...
83. (天禹)数据代理：通过一个对象代理对另一个对象中属性的操作(读/写)。
...
87. 这三个地方的东西会放在差值语法`{{}}`中：

    1. data
    2. props
    3. computed

88. 所有指令相关的 this 指向的是 window，如：自定义指令。

89. vue-CLI 中 CLI指：Command Line Interface 命令行接口工具。

90. 如果修改数据中的一个属性值，页面会重新渲染，说明该属性时响应式属性。
