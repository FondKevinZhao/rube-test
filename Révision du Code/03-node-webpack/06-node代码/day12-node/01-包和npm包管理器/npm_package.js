...
* 2.包的结构：
  ...
  4.npm常用的命令：
      ...
      一、安装：
            ...
            6.npm install xxxx -g:全局安装xxxx包（一般来说有指令的包，要进行全局安装，全局安装之后的包，在任意位置都能使用） C:\Users\username\AppData\Roaming\npm\node_modules
      ...
      二、移除：
            ...
            1.npm remove xxx
            （从当前的工程，移除指定的包）注意:千万不要在node_modules中手动移除
...
// 1. 引入要使用的第三方的包
// 注意:当引入第三方包的时候,会自动去当前项目的根目录找node_modules,找到了,就进去找uniq包,然后找到对应的js文件,去执行,并且返回导出的内容.如果当前目录下没有node_modules.就沿着目录层级一直往上找,直接当前盘符的根目录.如果还没有就报错,报错信息就是找不到指定包
