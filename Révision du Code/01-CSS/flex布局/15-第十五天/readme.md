...
* 移动web浏览
  ...
  `20世纪90年代中期`(`1990~1999`)，手机访问因特网不具备显示实际网站的能力，只能显示文本数据（使用最基本的HTML格式显示天气预报、股票报告、比赛得分）。

  `2005年`左右在更加先进的智能手机的设备上，移动浏览器才能够显示使用了CSS2和JavaScript技术的`"真正网页"`。
...
这个的意思指的是匹配屏幕设备并且它的最小宽度是600px，最大的宽度是800px时才会引入xxx.css
