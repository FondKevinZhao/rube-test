...
如果不会，就打开swiper的官网，然后点击开始使用，把那一页看完就会了。结构都是从官网复制来的。
...
其实 swiper 也可以写在 updated 里面，那么为什么选择 watch + nextTick 而不是 updated 呢？
...
- 返回值：返回一个新的 promise 对象。
  ...
  这个新的 promise 对象是成功的还是失败的，只有所有的 promise 都成功才成功，才算成功。只要有一个失败了，就直接失败。
...
2. Nginx帮我们做代理转发，因为生产环境没有DevServer

安装Nginx命令(命令没有写错)：yum install nginx //centOS里面安装Nginx
