...
<ul>
<li>a(20) // 10 20</li>
<li>a.call({ b: 20 }, 30) // 20 30</li>
<li>a.apply({ b: 20 }, [40]) // 30 40</li>
</ul> 
</details>
...
<summary>答案</summary> 
<ul>
<li>console.log(obj.prop.getFullname()) // 汤姆</li>
<li>var test = obj.prop.getFullname</li>
<li>console.log(test())  // 李雷</li>
</ul> 
</details>
...
<summary>答案</summary> 
<ul>
<li>let res = parseInt('x8x8') 
console.log('res', res) 结果为NaN，注意了，NaN加减乘除任何数都得NaN</li>
<li>lparseInt('x8x8') + parseFloat('8')的结果是NaN</li>
</ul> 
</details>
...
**object.create()：**创建一个新对象，使用现有的对象来提供新创建的对象的`__proto__`。 --MDN
...
<summary>答案</summary> 
<ul>
<li>lconsole.log(data[0].id, data[0].name) // 1 '一级菜单'</li>
<li>console.log(data[0].children[0].id, data[0].children[0].name) // 2 '二级菜单'</li>
<li>console.log(data[0].children[0].children[0].id, data[0].children[0].children[0].name) // 3 '三级菜单'</li>
</ul> 
</details>
...
使用 Websocket，通信的标签页连接同一个服务器，发送消息到服务器后，服务器推送消息给所有连接的[客户端](https://www.zhihu.com/search?q=客户端&search_source=Entity&hybrid_search_source=Entity&hybrid_search_extra={"sourceType"%3A"answer"%2C"sourceId"%3A2416694656})。
