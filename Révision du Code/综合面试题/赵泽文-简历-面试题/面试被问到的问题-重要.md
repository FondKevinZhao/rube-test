...
1. get上传的数据, 会出现在浏览器的地址栏中。 post 存放在请求主体中(f12在Network中可以查看) 。

2. get 不安全，post 相对安全。

3. get 一般用于获取数据，post 一般用于获取数据。

4. get 请求的数据量有限制(浏览器地址栏有大小，Chrome是4kb，不同的浏览器的大小限制是不一样的)，post 请求上传的数据理论上没有限制(实际上服务器会对上传的数据，进行限制)。
...
也就是说，我 git commit -m '这里面的内容，我上次提交打错字了，想只改变这个的内容'
